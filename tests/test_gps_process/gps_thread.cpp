...
// GPS test sentences
char test_gps_str[] = "$GPRMC,201547.000,A,3014.5527,N,09749.5808,W,0.24,163.05,040109,,*1A\r\n$GPGGA,201548.000,3014.5529,N,09749.5808,W,1,07,1.5,225.6,M,-22.5,M,18.8,0000*78\r\n";
size_t test_gps_str_len = strlen(test_gps_str);
...
os_thread_return_t gpsUpdateLoop(void* param)
{
  ...
  for(;;) {
    ...
    // get next character of test sentence
    char c = test_gps_str[i];
    i++; // increment and wrap around
    if (i == test_gps_str_len)
      i = 0;
